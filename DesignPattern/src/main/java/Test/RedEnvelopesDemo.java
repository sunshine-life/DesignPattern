...
public class RedEnvelopesDemo {
    ...
    public List<Integer> splitRedPackets(int money,int count){
        ...
        max = max>MAXMONEY *100? MAXMONEY*100 : max;
        //分配红包
    ...
    public static void main(String[] args) {
        ...
        //单位是分
        List<Integer> result = dd.splitRedPackets(199, 20);
        List<Float> list = new ArrayList<>();
        result.forEach(e -> list.add((float)(e/100.00)) );
        System.out.println(list);
        float total= list.stream().map((cost) -> cost).reduce((sum, cost) -> sum+ cost).get();
